...
import 'package:firebase_phone_auth/services/auth.dart';
...
class PhoneAuthDataProvider with ChangeNotifier {
  ...
  _startAuth() {
    ...
    final PhoneVerificationFailed verificationFailed = (AuthException authException) {
      ...
      if (authException.message.contains('not authorized'))
        ...
        _addStatusMessage('Please check your internet connection and try again');
      else
        _addStatusMessage('Something has gone wrong, please try later ' + authException.message);
    };
  ...
  void verifyOTPAndLogin({String smsCode}) async {
    ...
    FireBase.auth.signInWithCredential(_authCredential)
      .then((AuthResult result) async {
        ...
        if (onError != null) onError();
        _addStatus(PhoneAuthState.Error);
        _addStatusMessage('Something has gone wrong, please try later(signInWithPhoneNumber) $error');
      });
  }
